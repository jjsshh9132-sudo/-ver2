...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F8FAFC] text-slate-900 antialiased font-sans">
      ...
      <aside className="w-full md:w-[340px] bg-white border-r border-slate-200 flex flex-col shrink-0 shadow-xl overflow-y-auto no-scrollbar h-screen sticky top-0 z-20 print:hidden">
        {/* Banner Header */}
        <div className="p-6 bg-gradient-to-br from-indigo-700 to-indigo-900 text-white">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold opacity-70">비파괴 검사 수량 산출 프로</p>
            </div>
          </div>
          
          <button 
            onClick={addFacility}
            className="w-full py-3 bg-white text-indigo-900 rounded-xl text-xs font-black shadow-lg hover:bg-indigo-50 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
          >
        ...
        <div className="p-4 space-y-4">
          <section>
            <div className="flex items-center gap-2 px-2 mb-3">
              ...
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">시설물 선택 배너</label>
            </div>
            ...
            <div className="space-y-2">
              {appData.facilities.map(f => (
                <div 
                  ...
                  className={`group relative flex flex-col p-4 rounded-2xl border-2 transition-all cursor-pointer overflow-hidden ${
                    appData.currentId === f.id 
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <input 
                      ...
                      className="bg-transparent border-none text-[13px] font-black text-slate-800 focus:ring-0 p-0 w-full outline-none"
                    />
                    <button 
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${
                      f.selectedType === 'BRIDGE' ? 'bg-blue-100 text-blue-700' :
                      f.selectedType === 'TUNNEL' ? 'bg-emerald-100 text-emerald-700' :
          ...
          <section>
            <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 px-2 tracking-widest">구조물 형식 변경</label>
          ...
          <section className="space-y-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest flex items-center gap-2">
              <Settings2 className="w-3 h-3" /> 상세 설정
            </label>
            <div>
              ...
              <select 
                ...
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
              >
          ...
          <section className="pt-2">
            <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 px-2 tracking-widest">세부 제원 입력</label>
            <div className="space-y-4">
              {activeFacility.selectedType === 'BRIDGE' && (
                <div className="space-y-3">
                  {activeFacility.bridgeSpans.map((span, i) => (
                    <div key={span.id} className="p-4 bg-white border border-slate-200 rounded-2xl space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded uppercase">Span Group #{i+1}</span>
                      ...
                      <select 
                        ...
                        className="w-full bg-slate-50 border border-slate-100 rounded-xl px-2 py-1.5 text-[11px] font-bold outline-none"
                      >
              ...
              {activeFacility.selectedType === 'TUNNEL' && (
                <div className="space-y-3">
                  {activeFacility.tunnelSections.map((sec, i) => (
                    <div key={sec.id} className="p-4 bg-white border border-slate-200 rounded-2xl space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded uppercase">Section #{i+1}</span>
                      ...
                      <div className="flex gap-1">
                        <button 
                          ...
                          className={`flex-1 py-1.5 rounded-lg border text-[9px] font-black transition-all ${sec.concreteType === 'REINFORCED' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 text-slate-400 border-slate-100'}`}
                        >
                          철근
                        </button>
                        <button 
                          onClick={() => updateSection('tunnelSections', sec.id, 'concreteType', 'PLAIN')}
                          className={`flex-1 py-1.5 rounded-lg border text-[9px] font-black transition-all ${sec.concreteType === 'PLAIN' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 text-slate-400 border-slate-100'}`}
                        >
                          무근
                        </button>
                      </div>
              ...
              {activeFacility.selectedType === 'RETAINING_WALL' && (
                <div className="space-y-3">
                  {activeFacility.retainingWallSections.map((sec, i) => (
                    <div key={sec.id} className="p-4 bg-white border border-slate-200 rounded-2xl space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-black text-orange-600 bg-orange-50 px-2 py-0.5 rounded uppercase">Section #{i+1}</span>
                      ...
                      <div className="grid grid-cols-3 gap-1">
                        {(['CONCRETE', 'REINFORCED_SOIL', 'STONE'] as const).map(type => (
                          <button 
                            ...
                            className={`py-1.5 rounded-lg border text-[8px] font-black transition-all ${sec.type === type ? 'bg-orange-600 text-white border-orange-600' : 'bg-slate-50 text-slate-400 border-slate-100'}`}
                          >
        ...
        <div className="mt-auto p-6 border-t border-slate-100 bg-slate-50/50">
          <button onClick={resetData} className="w-full py-2 text-[10px] font-black text-slate-400 hover:text-red-500 transition-all flex items-center justify-center gap-1.5">
            <RotateCcw className="w-3 h-3" /> 모든 데이터 초기화
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-10 print:p-0">
        <div className="max-w-5xl mx-auto space-y-8">
          {/* Header Card */}
          <div className="bg-white rounded-[32px] border border-slate-200 p-8 md:p-10 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden print:border-none print:shadow-none">
            <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-50 rounded-full -mr-40 -mt-40 opacity-40 print:hidden"></div>
            <div className="flex items-center gap-6 relative z-10 w-full">
              <div className="w-16 h-16 bg-indigo-600 rounded-[22px] flex items-center justify-center shadow-2xl shadow-indigo-200 text-white shrink-0">
                <Calculator className="w-8 h-8" />
              </div>
              <div className="flex-1 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{activeFacility.name}</h1>
                ...
                <div className="flex items-center gap-3">
                  <div className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-2xl text-[11px] font-black border border-indigo-100">
                    {STRUCTURE_INFO[activeFacility.selectedType].name}
                  </div>
                  <div className="px-4 py-2 bg-slate-100 text-slate-700 rounded-2xl text-[11px] font-black border border-slate-200">
            ...
            <button 
              ...
              className="flex items-center gap-2.5 px-6 py-3 bg-slate-900 text-white rounded-[20px] text-sm font-black hover:bg-black transition-all shadow-xl hover:shadow-2xl active:scale-95 relative z-10 shrink-0 print:hidden"
            >
              <Printer className="w-4 h-4" /> 리포트 출력 / PDF
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:hidden">
            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center"><Construction className="w-5 h-5" /></div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="w-10 h-10 bg-orange-50 text-orange-600 rounded-xl flex items-center justify-center"><Info className="w-5 h-5" /></div>
          ...
          {/* Results Table Card */}
          <div className="bg-white rounded-[32px] border border-slate-200 shadow-2xl overflow-hidden print:border-slate-300 print:shadow-none">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  <tr className="bg-slate-50/80">
                    <td colSpan={isBridge ? 6 : 4} className="p-3 px-6 text-[11px] font-black text-slate-600 flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-indigo-600 rounded-full"></div> 필수 과업 항목
                    </td>
                  </tr>
                  {TEST_RULES[activeFacility.selectedType].filter(r => getCategory(r.id, activeFacility) === 'BASIC').map(rule => {
                    ...
                    return (
                      <tr key={rule.id} className="hover:bg-indigo-50/20 transition-colors group">
                            ...
                            <td className="p-3 border-r border-slate-200 text-center bg-indigo-50/10">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <input 
                                  ...
                                  className="w-14 bg-white border border-indigo-100 rounded-lg text-center text-[11px] font-black text-indigo-600 shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none print:bg-transparent print:border-none print:shadow-none"
                                />
                              </div>
                            </td>
                            <td className="p-3 border-r border-slate-200 text-center bg-indigo-50/10">
                        ...
                        <td className="p-3 text-[10px] text-slate-400 text-center italic font-medium">지침 기준 준수</td>
                      </tr>
                    );
                  })}
                  ...
                  <tr className="bg-slate-50/80">
                    <td colSpan={isBridge ? 6 : 4} className="p-3 px-6 text-[11px] font-black text-slate-600 flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-slate-400 rounded-full"></div> 선택 과업 (판단 필요)
                    </td>
                  </tr>
                  {TEST_RULES[activeFacility.selectedType].filter(r => getCategory(r.id, activeFacility) === 'OPTIONAL').map(rule => {
                    ...
                    return (
                      <tr key={rule.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="p-4 border-r border-slate-200 font-bold text-slate-600 text-[11px] text-center group-hover:text-slate-900 transition-colors">{rule.name}</td>
                        {isBridge ? (
                          <>
                            <td className="p-3 border-r border-slate-200 text-[10px] text-slate-400 text-center whitespace-pre-line leading-relaxed">{res.upperCriteria || '-'}</td>
                            <td className="p-3 border-r border-slate-200 text-[10px] text-slate-400 text-center whitespace-pre-line leading-relaxed">{res.lowerCriteria || '-'}</td>
                            <td className="p-2 border-r border-slate-200 text-center">
                              <div className="flex items-center justify-center gap-2">
                                <span className="text-slate-300 font-medium text-[11px]">{res.upperQty}</span>
                                <span className="text-slate-200 text-[10px]">|</span>
                                <input 
                                  type="text" 
                                  value={valU} 
                                  onChange={(e) => updateImplQty(rule.id, 'upper', e.target.value)}
                                  className="w-14 bg-slate-50 border border-slate-100 rounded-lg text-center text-[11px] font-bold text-slate-500 outline-none print:bg-transparent print:border-none"
                                />
                              </div>
                            </td>
                            <td className="p-2 border-r border-slate-200 text-center">
                              <div className="flex items-center justify-center gap-2">
                                <span className="text-slate-300 font-medium text-[11px]">{res.lowerQty}</span>
                                <span className="text-slate-200 text-[10px]">|</span>
                                <input 
                                  type="text" 
                                  value={valL} 
                          ...
                          <>
                            <td colSpan={2} className="p-3 border-r border-slate-200 text-[10px] text-slate-400 text-center whitespace-pre-line leading-relaxed">{res.upperCriteria || '-'}</td>
                        ...
                        <td className="p-3 text-[10px] text-slate-400 text-center italic font-medium">현장 여건 고려</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-[32px] p-8 text-white shadow-2xl relative overflow-hidden print:hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-32 -mt-32"></div>
            <div className="flex items-start gap-6 relative z-10">
              <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center shrink-0"><Info className="w-6 h-6 text-indigo-300" /></div>
              <div className="space-y-4">
                <h3 className="text-lg font-black tracking-tight text-indigo-300 underline underline-offset-8 decoration-indigo-300/30">산출 근거 및 유의사항</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-3 text-[12px] font-medium text-slate-300 leading-relaxed">
                  <div className="flex items-start gap-2"><span className="text-indigo-400 font-black">•</span> 교량 연장(L)에 따른 보정계수(r)가 자동으로 적용되었습니다.</div>
                  <div className="flex items-start gap-2"><span className="text-indigo-400 font-black">•</span> 터널의 경우 철근/무근 콘크리트 형식에 따라 시험 항목이 필터링됩니다.</div>
                  <div className="flex items-start gap-2"><span className="text-indigo-400 font-black">•</span> 옹벽은 평가단위(기본 20m)를 기준으로 산정되며, 석축의 경우 암반풍화도 등이 포함됩니다.</div>
                  <div className="flex items-start gap-2"><span className="text-indigo-400 font-black">•</span> 금회 수량은 기준 수량을 바탕으로 자동 입력되며, 현장 여건에 맞춰 수정 가능합니다.</div>
                </div>
              </div>
            </div>
          </div>
